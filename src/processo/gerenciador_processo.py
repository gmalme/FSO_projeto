...
from filas.fila_processos import FilaProcessos
...
class ProcessoGerenciador:
    ...
    def __context_switching(self, processo):
        """ Realizes the context switching between two processos

        Args:
            processo (Processo): processo that will be executed (allocated in memoria)

        Returns:
            Processo: the last current processo
        """
        if processo[0]:
    ...
    def fila_atual_thread(self):
        """Thread function that will be waiting for the arriving real-time processos"""
        while True:
    ...
    def fila_thread_usuarios(self):
        """Thread function that will be waiting for the arriving usuarios processos"""
        while True:
